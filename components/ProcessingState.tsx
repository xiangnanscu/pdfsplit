...
export const ProcessingState: React.FC<Props> = ({
  ...
  if (status === ProcessingStatus.IDLE || status === ProcessingStatus.COMPLETED)
    return null;
    ...
    <div className="fixed bottom-6 right-6 z-[190] w-80 md:w-96 shadow-2xl border-2 border-slate-100 bg-white rounded-2xl p-5 animate-[fade-in_0.3s_ease-out] flex flex-col gap-4">
      {isError ? (
        <div className="flex flex-col gap-2">
          ...
          <button
            onClick={onClose}
            className="bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-bold py-2 rounded-lg mt-2 transition-colors"
          >
            Dismiss
          </button>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="shrink-0">
            <CircularProgress
              ...
              strokeWidth={4}
              colorClass={
                currentRound > 1 ? "text-orange-500" : "text-blue-600"
              }
              label={
            ...
            <div className="flex items-center gap-2 text-[10px] font-bold text-slate-500 bg-slate-50 px-2 py-1 rounded-lg border border-slate-100 self-start inline-flex">
              ...
              {onAbort && (
                <>
                  ...
                  <button
                    onClick={onAbort}
                    className="text-red-500 hover:text-red-600 uppercase"
                  >
                    Stop
                  </button>
                </>
