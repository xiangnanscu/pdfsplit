...
export const RefinementModal: React.FC<Props> = ({
  ...
  const [localSettings, setLocalSettings] =
    useState<CropSettings>(initialSettings);
      ...
      <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden scale-100 animate-[scale-in_0.2s_cubic-bezier(0.175,0.885,0.32,1.275)]">
        <div className="p-6 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
          <div>
            ...
            <p className="text-slate-400 text-xs font-bold truncate max-w-[250px]">
              {fileName}
            </p>
          </div>
          <button
        ...
        <div className="p-8 space-y-6">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3 relative group">
              <input
                type="number"
                value={localSettings.cropPadding}
                onChange={(e) =>
                  setLocalSettings((prev) => ({
                    ...prev,
                    cropPadding: Number(e.target.value),
                  }))
                }
                className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500 transition-all text-base"
              />
              <span className="absolute right-5 text-xs text-slate-400 font-black uppercase select-none">
                px
              </span>
            </div>
            <p className="text-[10px] text-slate-400">
              Buffer around the AI detection box.
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3 relative group">
              <input
                type="number"
                value={localSettings.canvasPadding}
                onChange={(e) => {
                  const v = Number(e.target.value);
                  setLocalSettings((p) => ({ ...p, canvasPadding: v }));
                }}
                className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500 transition-all text-base"
              />
              <span className="absolute right-5 text-xs text-slate-400 font-black uppercase select-none">
            ...
            <p className="text-[10px] text-slate-400">
              Aesthetic whitespace added to the final image.
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3 relative group">
              <input
                type="number"
                value={localSettings.mergeOverlap}
                onChange={(e) =>
                  setLocalSettings((p) => ({
                    ...p,
                    mergeOverlap: Number(e.target.value),
                  }))
                }
                className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500 transition-all text-base"
              />
            ...
            <p className="text-[10px] text-slate-400">
              Vertical overlap when stitching split questions.
            </p>
          </div>
