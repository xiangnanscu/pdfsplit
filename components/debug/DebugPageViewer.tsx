...
export const DebugPageViewer: React.FC<Props> = ({
  ...
  const handleWheel = (e: React.WheelEvent) => {
    ...
    if (isAtTop && hasPrevFile && e.deltaY < 0) {
      ...
      if (
        pullDelta !== 0 &&
        ((isAtTop && !hasPrevFile) || (isAtBottom && !hasNextFile))
      ) {
        setPullDelta(0);
      }
    }
  ...
  const handleSvgMouseDown = useCallback(
    (e: React.MouseEvent, side: "left" | "right" | "top" | "bottom") => {
      e.stopPropagation();
      e.preventDefault();
    ...
    >
      ...
      >
        <div className="flex flex-col items-center">
          <svg
            className={`w-6 h-6 text-blue-400 transition-transform duration-300 ${Math.abs(pullDelta) > PULL_THRESHOLD ? "rotate-180 scale-110" : ""}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
          ...
          <span className="text-white text-xs font-bold uppercase tracking-widest mt-1">
            {Math.abs(pullDelta) > PULL_THRESHOLD
              ? "Release to Previous"
              : "Pull for Previous"}
          </span>
        </div>
        ...
        {pages.map((page) => (
          ...
          >
            <div className="flex justify-between items-end mb-2 px-1">
              <div>
                <h3 className="text-slate-400 font-bold text-xs uppercase tracking-widest truncate max-w-[300px]">
                  {page.fileName}
                </h3>
                <div className="flex gap-2 mt-1">
                  <span className="bg-blue-900/30 text-blue-300 text-[10px] font-black uppercase px-2 py-0.5 rounded border border-blue-800/50">
            ...
            <div className="relative w-full shadow-2xl shadow-black/50 rounded-sm overflow-hidden bg-white ring-1 ring-slate-800 select-none">
              ...
              <svg
                ref={
                  selectedDetection?.pageNumber === page.pageNumber
                    ? svgRef
                    : undefined
                }
              ...
              >
                {page.detections.map((det, detIdx) => {
                  ...
                  const isGrouped =
                    columnInfo &&
                    selectedDetection?.pageNumber === page.pageNumber &&
                    columnInfo.indices.includes(detIdx);
                  const boxes = (
                    Array.isArray(det.boxes_2d[0])
                      ? det.boxes_2d
                      : [det.boxes_2d]
                    ...
                    <>
                      {columnInfo && (
                        <>
                          <line
                            x1={
                              draggingSide === "left" && dragValue !== null
                                ? dragValue
                                : columnInfo.initialLeft
                            }
                            y1="0"
                            x2={
                              draggingSide === "left" && dragValue !== null
                          ...
                          <line
                            x1={
                              draggingSide === "right" && dragValue !== null
                                ? dragValue
                                : columnInfo.initialRight
                            }
                            y1="0"
                            x2={
                              draggingSide === "right" && dragValue !== null
                      ...
                      <>
                        <line
                          x1={selectedBoxCoords.xmin - 20}
                          y1={
                            draggingSide === "top" && dragValue !== null
                              ? dragValue
                              : selectedBoxCoords.ymin
                          }
                          x2={selectedBoxCoords.xmax + 20}
                          y2={
                            draggingSide === "top" && dragValue !== null
                              ? dragValue
                              : selectedBoxCoords.ymin
                      ...
                      <>
                        <line
                          x1={selectedBoxCoords.xmin - 20}
                          y1={
                            draggingSide === "bottom" && dragValue !== null
                              ? dragValue
                              : selectedBoxCoords.ymax
                          }
                          x2={selectedBoxCoords.xmax + 20}
                          y2={
                            draggingSide === "bottom" && dragValue !== null
                      ...
                      {draggingSide && dragValue !== null && (
                        <g>
                          ...
                          <text
                            ...
                            textAnchor="middle"
                          >
                            {draggingSide === "left" || draggingSide === "right"
                              ? "X: "
                              : "Y: "}
