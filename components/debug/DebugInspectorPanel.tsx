...
export const DebugInspectorPanel: React.FC<Props> = ({
  ...
  const PreviewCard = ({ title, url, color, desc }: { title: string, url?: string, color: string, desc: string }) => (
     <div className="space-y-2">
        ...
        <div className={`bg-slate-950 rounded-2xl border border-${color}-900/30 p-4 shadow-xl relative group overflow-hidden min-h-[120px]`}>
            {url ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-slate-700 min-h-[100px]">
                    {isGenerating ? (
                        <div className={`w-5 h-5 border-2 border-${color}-500 border-t-transparent rounded-full animate-spin`}></div>
                    ) : (
        ...
        {selectedDetection ? (
          <div className="space-y-8 animate-[fade-in_0.3s_ease-out] pb-12">
              ...
              <div className="pt-4 border-t border-slate-800">
                 ...
                 <div className="grid grid-cols-2 gap-2 text-[10px]">
                    <div className={`bg-slate-800/50 p-2 rounded text-center border ${draggingSide === 'top' ? 'border-green-500 text-green-400' : 'border-slate-800 text-slate-300'}`}>
                        Y-MIN: {selectedDetection ? Math.round((Array.isArray(selectedDetection.boxes_2d[0]) ? selectedDetection.boxes_2d[0][0] : selectedDetection.boxes_2d[0]) as number) : '-'}
                    </div>
                    <div className={`bg-slate-800/50 p-2 rounded text-center border ${draggingSide === 'left' ? 'border-blue-500 text-blue-400' : 'border-slate-800 text-slate-300'}`}>
                        X-MIN: {selectedDetection ? Math.round((Array.isArray(selectedDetection.boxes_2d[0]) ? selectedDetection.boxes_2d[0][1] : selectedDetection.boxes_2d[1]) as number) : '-'}
                    </div>
                    <div className={`bg-slate-800/50 p-2 rounded text-center border ${draggingSide === 'bottom' ? 'border-green-500 text-green-400' : 'border-slate-800 text-slate-300'}`}>
                        Y-MAX: {selectedDetection ? Math.round((Array.isArray(selectedDetection.boxes_2d[0]) ? selectedDetection.boxes_2d[0][2] : selectedDetection.boxes_2d[2]) as number) : '-'}
                    </div>
                    <div className={`bg-slate-800/50 p-2 rounded text-center border ${draggingSide === 'right' ? 'border-blue-500 text-blue-400' : 'border-slate-800 text-slate-300'}`}>
                        X-MAX: {selectedDetection ? Math.round((Array.isArray(selectedDetection.boxes_2d[0]) ? selectedDetection.boxes_2d[0][3] : selectedDetection.boxes_2d[3]) as number) : '-'}
                    </div>
                 </div>
              </div>
          </div>
          ...
          <div className="h-full flex flex-col items-center justify-center text-center opacity-40">
              <div className="w-20 h-20 bg-slate-800 rounded-full flex items-center justify-center mb-6">
                  <svg className="w-10 h-10 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5M7.188 2.239l.777 2.897M5.136 7.965l-2.898-.777M13.95 4.05l-2.122 2.122m-5.657 5.656l-2.12 2.122" /></svg>
              </div>
              <p className="text-slate-500 text-sm font-bold max-w-[200px]">Select a box to analyze the 4 processing stages.</p>
          </div>
        )}
      </div>
    </div>
