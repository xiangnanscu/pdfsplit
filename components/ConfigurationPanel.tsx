          ...
          <div className="space-y-8">
             <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   {[
                    { id: 'gemini-3-flash-preview', name: 'Gemini 3.0 Flash', desc: 'Fastest & Cost-effective' },
                    { id: 'gemini-3-pro-preview', name: 'Gemini 3.0 Pro', desc: 'High Reasoning Power' }
                   ].map(model => (
                      <button
                        key={model.id}
                      ...
                      >
                        <div className="flex justify-between items-center mb-1">
                          <span className={`font-black ${selectedModel === model.id ? 'text-blue-700' : 'text-slate-700'}`}>{model.name}</span>
                          {selectedModel === model.id && <div className="w-3 h-3 bg-blue-600 rounded-full"></div>}
                        </div>
                        <p className="text-xs text-slate-400 font-bold">{model.desc}</p>
                      </button>
                   ))}
                </div>
             </div>
             ...
             <div className="space-y-6 pt-6 border-t border-slate-100">
                ...
                {setBatchSize && batchSize !== undefined && (
                  <div className="space-y-3">
                    <div className="flex justify-between">
                        <label className="font-bold text-slate-700">Image Processing Batch Size</label>
                        <div className="flex items-center gap-2">
                          <span className="font-black text-blue-600 bg-blue-50 px-3 py-1 rounded-lg text-xs">{batchSize} Threads</span>
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Max: {detectedCores}</span>
                        </div>
                    </div>
                    <input 
                        type="range" 
                ...
                <div className="flex items-center justify-between">
                    ...
                    >
                      <div className={`bg-white w-6 h-6 rounded-full shadow-md transform transition-transform ${useHistoryCache ? 'translate-x-6' : ''}`}></div>
                    </div>
                </div>
             </div>

             <div className="space-y-6 pt-6 border-t border-slate-100">
                <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest">Global Crop Defaults</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                   <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-500">Crop Padding (px)</label>
                      <input 
                        type="number" 
                        value={cropSettings.cropPadding}
                        onChange={(e) => setCropSettings({...cropSettings, cropPadding: parseInt(e.target.value) || 0})}
                   ...
                   <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-500">Inner Padding (px)</label>
                      <input 
                        type="number" 
                        value={cropSettings.canvasPadding}
                        onChange={(e) => setCropSettings({...cropSettings, canvasPadding: parseInt(e.target.value) || 0})}
                        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500"
                      />
                   </div>
                   <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-500">Merge Overlap (px)</label>
                      <input 
                        type="number" 
                        value={cropSettings.mergeOverlap}
                        onChange={(e) => setCropSettings({...cropSettings, mergeOverlap: parseInt(e.target.value) || 0})}
                        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500"
                      />
                   </div>
                </div>
