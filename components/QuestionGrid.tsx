      ...
      <div style={style} className="px-4 md:px-8 py-4 z-10">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white/80 backdrop-blur-md p-4 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={() => onDebug(row.fileName)}
              className="px-3 py-1.5 rounded-lg text-xs font-bold text-slate-500 hover:text-blue-600 hover:bg-blue-50 border border-slate-200 bg-white transition-colors"
            >
              Debug
            </button>
            <button
              onClick={() => onRefine(row.fileName)}
              className="px-3 py-1.5 rounded-lg text-xs font-bold text-slate-500 hover:text-blue-600 hover:bg-blue-50 border border-slate-200 bg-white transition-colors"
            >
              Refine
            </button>
          ...
          >
            <div className="px-4 py-2 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div className="flex items-center gap-2">
                ...
                {q.analysis && (
                  <span className="text-[9px] font-bold text-white bg-slate-800 px-1.5 py-0.5 rounded shadow-sm">
                    AI
                  </span>
                )}
              </div>
...
export const QuestionGrid: React.FC<Props> = ({
  ...
  const generateZip = async (targetFileName?: string) => {
    ...
    const fileNames = targetFileName
      ? [targetFileName]
  ...
  const hasNext =
    selectedImage && questions.indexOf(selectedImage) < questions.length - 1;
    ...
    <>
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-4">
                <p className="text-slate-500 font-semibold flex items-center gap-2 text-sm">
                  ...
                  Extracted {questions.length} questions from{" "}
                  {sortedFileNames.length} files
                </p>
              </div>
            </div>
      ...
      {selectedImage && (
        ...
        >
          ...
          <div
            className="relative max-w-7xl w-full h-[95vh] flex flex-col p-6 md:px-16 md:py-10"
              ...
              <div className="flex flex-col">
                <h2 className="text-3xl font-black tracking-tight flex items-center gap-3">
                  ...
                  {selectedImage.analysis && (
                    <span className="text-xs bg-slate-800 border border-slate-600 px-2 py-1 rounded text-slate-200 font-bold">
                      已解析
                    </span>
                  )}
                </h2>
